...
def _concrete_host_size(size):
    """Resolve symbolic sizes to a list of Python ints for SpyreTensorLayout(host_size, dtype)."""
    sizevars = getattr(V.graph, "sizevars", None)
    if sizevars is None:
        return [int(s) for s in size]
    replacements = getattr(sizevars, "precomputed_replacements", None) or {}
    out = []
    for s in size:
        if isinstance(s, int):
            out.append(s)
            continue
        sub = sympy_subs(s, replacements) if replacements else s
        # After substitution we may get sympy.Integer or still have a symbol; try size_hint.
        if hasattr(sizevars, "size_hint"):
            out.append(int(sizevars.size_hint(sub)))
        elif hasattr(sub, "is_Integer") and sub.is_Integer:
            out.append(int(sub))
        else:
            try:
                out.append(int(sub))
                ...
                raise Unsupported(
                    f"clone/contiguous: symbolic size {s} could not be resolved to int"
                )
    return out
    ...
    if len(args) == 1:
        ...
        match op:
            ...
            case aten.clone.default:
                ...
                # SpyreTensorLayout(host_size, dtype) requires concrete ints, not symbolic sizes.
                host_size = _concrete_host_size(output.size)
                stl = SpyreTensorLayout(host_size, output.dtype)

            case aten.contiguous.default:
                if is_sparse(x_stl):
                    raise Unsupported("contiguous on sparse tensor")

                # Same layout as clone: force default (contiguous) layout.
                host_size = _concrete_host_size(output.size)
                stl = SpyreTensorLayout(host_size, output.dtype)
